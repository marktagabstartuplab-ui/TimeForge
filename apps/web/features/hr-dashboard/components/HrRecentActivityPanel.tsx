...
import { getHrRecent } from "../api/hr-dashboard.service";
...
const TABS: { value: Tab; label: string }[] = [
  ...
  { value: "registrations", label: "Onboarding" },
];
...
export function HrRecentActivityPanel() {
  const [tab, setTab] = useState<Tab>("audit");
  const { data, isLoading, isError } = useQuery({
    queryKey: ["hr-dashboard", "recent"],
    queryFn: getHrRecent,
  });
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <EmptyState message="Couldn't load recent activity. Try again shortly." />
      ) : (
        <>
          {tab === "audit" ? (
              ...
              <ul className="flex flex-col divide-y divide-[#c3c6d2]/40">
                {data.payrollGenerations.map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-3 py-2.5 text-sm">
                    <span className="font-medium text-brand-ink">{actionLabel(p.period.type)} payroll generated</span>
          ...
          {tab === "registrations" ? (
            data && data.userRegistrations.length > 0 ? (
              ...
              <EmptyState message="No new hires onboarded yet." />
            )
          ) : null}
        </>
      )}
