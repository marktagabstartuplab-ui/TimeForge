...
import { startOfOrgMonth, startOfOrgWeek } from '@timeforge/shared';
...
export class OrganizationService {
  ...
  async getDashboard(tenantId: string, organizationId: string) {
    ...
    // Month and week boundaries follow the organization's timezone. These were
    // derived with setHours/getMonth, i.e. the API host's timezone — UTC on
    // Railway, local on a developer machine — so the same request answered
    // differently depending on where it ran.
    const now = new Date();
    const timeZone = await this.timeZones.forOrganization(tenantId, organizationId);
    const startOfMonth = startOfOrgMonth(now, timeZone);
    // Sunday-start week, matching the previous behaviour.
    const startOfWeek = startOfOrgWeek(now, timeZone, 0, 0);
