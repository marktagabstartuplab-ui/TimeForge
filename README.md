...
| 3 | Daily Scrum | ⚠️ | Yesterday/Today/Blockers/Notes all captured; supervisor comments persist. "Recurring operational issues" is only a manual one-off flag (`scrum.service.ts:flagScrumEntry`), not automatic pattern detection — the AI `BLOCKER_DETECTION` feature covers this via LLM analysis instead of a rules engine. **BUG-U fixed:** after EOD submission, all commitments (KPI-linked and non-KPI) are now marked `COMPLETED` and locked — Edit/Delete are hidden for completed tasks in `ScrumTaskCard.tsx`; `scrum.service.ts` rejects plan edits/deletes on COMPLETED tasks (409); supervisor `unlockEntry` reopens both the day entry and the individual task rows back to `IN_PROGRESS`. |
...
| 6 | Payroll Preparation | ✅ | FIRST_HALF/SECOND_HALF/CUSTOM periods, all required summary fields, real PDF **and** Excel export (`payroll-export.processor.ts`). Supports **Hourly and Daily Rate compensation basis** (BUG-AW): `compensation_type` (`HOURLY`, `DAILY`), `daily_rate`, `days_per_week` configured per employee in Admin/Employee Profile, calculated as `days_worked × daily_rate` with partial days prorated. |
...
13. ✅ **Employee Relations & Discipline (BUG-BB)** — NTE (Notice to Explain) generator for HR with employee written-explanation submission (Twin-Notice Rule), Grievance Inbox restricted to `grievance:read_org`, and an offboarding Clearance Tracker routing IT/Finance/HR sign-offs. Final pay is held **per employee** while their clearance is `PENDING`/`IN_PROGRESS` — the held employee gets no line item and their timesheets stay `UNPAID` so a later run still pays them, while everyone else in the period is paid normally. Held employees are named on the report totals under `blockedByClearance`. Screens: `/hr/employee-relations` (HR) and `/my-records` (employee).
14. ✅ **13th-Month & De Minimis Trackers (BUG-BC)** — 13th-month tracker aggregates Jan 1–Dec 31 basic salary from generated line items and derives the payout as YTD ÷ 12; `de_minimis_benefits` stores per-employee non-taxable allowances clamped to BIR RR 11-2018 monthly ceilings. Payroll adds the prorated amount to net pay only — gross, taxable income and every contribution base are untouched — and the payslip (screen + PDF) shows it as its own line. Screen: `/hr/compensation`.
